...
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #9b7ed4, #c06090) !important;
...
.stTabs [aria-selected="true"] {
    background: rgba(155,126,212,0.15) !important;
    color: #6b4d90 !important;
}
...
def show_auth():
    ...
    with col2:
        ...
        st.markdown('<div class="auth-tagline">Your private space to reflect, grow, and understand yourself.</div>', unsafe_allow_html=True)
...
def show_sidebar():
    with st.sidebar:
        ...
        user = current_user()
        st.markdown(f"<span style='font-size:0.85rem;color:#9b84b0;'>✉️ {user.email}</span>", unsafe_allow_html=True)
        st.divider()

        pages = {
            "🏠 Home": "home",
            "✍️ New Entry": "new_entry",
            "📖 My Journal": "journal",
            "📊 Insights": "analytics",
        }
...
def show_home():
    ...
    st.markdown('<div class="page-subtitle">What\'s on your mind today?</div>', unsafe_allow_html=True)

    if st.button("✍️ Write Today's Entry", type="primary"):
    ...
    if not entries:
        ...
        <div class="mirror-card" style="text-align:center;padding:3rem;">
            ...
            <div style="font-family:'Playfair Display',serif;font-size:1.3rem;color:#4a3060;margin:0.8rem 0 0.4rem;">Your journal is empty</div>
            <div style="color:#9b84b0;font-size:0.9rem;">Write your first entry and let AI help you reflect.</div>
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    if len(entries) > 3:
        if st.button(f"View all {len(entries)} entries →"):
...
def show_new_entry():
    ...
    title_label = "Edit Entry" if editing else "New Entry"

    st.markdown(f'<div class="page-title">✍️ {title_label}</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-subtitle">Write freely. AI will help you reflect.</div>', unsafe_allow_html=True)

    defaults = editing or {}

    with st.form("entry_form", clear_on_submit=False):
        title = st.text_input("Title", value=defaults.get("title", ""), placeholder="Give today a title...")
        ...
        content = st.text_area("Your entry", value=defaults.get("content", ""),
                               placeholder="Start writing... let your thoughts flow.",
                               height=280)

        col1, col2 = st.columns(2)
        with col1:
            ai_on = st.checkbox("✨ Generate AI insights", value=True)
        with col2:
            submitted = st.form_submit_button(
                "💾 Save Entry" if editing else "✨ Save & Reflect",
                type="primary", use_container_width=True
            )

        if submitted:
            if not title.strip() or not content.strip():
                st.error("Please add a title and some content.")
            else:
                user = current_user()
                insights = {}
                if ai_on:
                    with st.spinner("🤖 Groq is reading your entry..."):
                        try:
    ...
    if mood:
        ...
        <div style="margin-top:1rem;display:flex;align-items:center;gap:0.6rem;">
            ...
            <span style="font-size:0.85rem;color:#9b84b0;">Currently feeling {mood}</span>
        </div>
        """, unsafe_allow_html=True)
...
def show_journal():
    ...
    st.markdown('<div class="page-title">📖 My Journal</div>', unsafe_allow_html=True)
    st.markdown('<div class="page-subtitle">All your reflections, in one place.</div>', unsafe_allow_html=True)
    ...
    # Filter by mood
    mood_filter = st.selectbox("Filter by mood", ["All moods"] + MOODS, key="mood_filter")
    if mood_filter != "All moods":
        entries = [e for e in entries if e.get("mood") == mood_filter]

    st.markdown(f"<div style='color:#9b84b0;font-size:0.85rem;margin-bottom:1rem;'>{len(entries)} entries</div>", unsafe_allow_html=True)
...
def _render_entry_card(entry: dict, compact: bool = False):
    ...
    if not compact:
        ...
        with col1:
            if st.button("📖 Read", key=f"view_{entry['id']}", use_container_width=True):
        ...
        if st.session_state.get(f"confirm_delete_{entry['id']}"):
            ...
            with cc1:
                if st.button("Yes, delete", key=f"yes_{entry['id']}", type="primary"):
        ...
        if st.button("Read →", key=f"home_view_{entry['id']}"):
            st.session_state.viewing_entry = entry
            st.session_state.page = "journal"
            st.rerun()
...
def _show_entry_detail(entry: dict):
    ...
    if entry.get("reflection_prompt"):
        ...
        <div class="reflection-box">
            <strong>💭 Reflection Prompt</strong><br><br>
...
def show_analytics():
    ...
    st.markdown('<div class="page-subtitle">Patterns in your emotional landscape.</div>', unsafe_allow_html=True)
    ...
    for col, (num, label) in zip(
        ...
        [
            ...
            (f"{stats['streak']}d", "Current Streak"),
            (stats["top_mood"], "Top Mood"),
            (f"{stats['avg_words']}w", "Avg Words"),
        ],
    ):
        with col:
            st.markdown(f"""
    ...
    if df.empty or len(df) < 1:
        st.info("Write a few entries to unlock your mood analytics! 🌱")
        return
    ...
    with col_b:
        st.markdown("**Words Written per Day**")
